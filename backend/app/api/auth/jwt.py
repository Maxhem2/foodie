...
async def login(form_data: OAuth2PasswordRequestForm = Depends()) -> Any:
    ...
    # Zugriffs- und Aktualisierungstoken erstellen und zurückgeben
    return {
...
# Endpunkt zum Aktualisieren des Zugriffstokens anhand des Aktualisierungstokens
@auth_router.post('/refresh', summary="Refresh token", response_model=TokenSchema)
async def refresh_token(refresh_token: str = Body(...)):
    try:
        # Entschlüsseln des Aktualisierungstokens und Validierung des Inhalts
        payload = jwt.decode(
    ...
    # Neues Zugriffs- und Aktualisierungstoken erstellen und zurückgeben
    return {
        "access_token": create_access_token(user.user_id),
        "refresh_token": create_refresh_token(user.user_id),
    }
